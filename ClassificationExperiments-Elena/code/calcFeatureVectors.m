% bird behaviour | PREPROCESSING
% merijn de bakker | merijn.debakker@gmail.com
% created: 11-2011 edited: 11-2011
%
% function calcFeatureVectors. Calculates the features as assumed by a trained
% classifier. 
%
% INPUT: A NxM matrix containing unlabelled data. Structure:
% [device,datetime,index,x,y,z,spd];
% OUTPUT: NxM matrix containing N vectors with M features. INFO is a cell
% array containing helpful information about the feature vector.
% DEPENDS: windowing
% NOTES: 
...
if ~isempty(windows)
    ...
    SPRM = cellfun(@position, {WM{:,1}}', 'uniformoutput', false);

    %correlation features
    CM = cellfun(@correlation, {WM{:,1}}', 'uniformoutput', false);

    %motionlessness feature matrix for window set
    MLM = cellfun(@motionlessness, {WM{:,1}}', 'uniformoutput', false);

    %fourier transform feature matrix for window set
    FM = cellfun(@FaFoTr, {WM{:,1}}', 'uniformoutput', false);
