...
function [F,lab,N,treeInfo] = tree_map_e(T,W)
...
for i = num:n
	...
	if tree(i,3) > 0
        ...
        %EDIT_0611_merijndebakker -- put the decision info in the matrix
        feature = tree(i,1);
        threshold = tree(i,2);
        nodIfSmall = tree(i,3);
        nodIfBig = tree(i,4);
        probs = tree(i,5:4+c);
        treeInfo(end+1,:) = [i, feature, threshold, nodIfSmall,probs];
        treeInfo(end+1,:) = [i, feature, threshold, nodIfBig, probs];
	...
	elseif tree(i,3) == 0
        ...
        %EDIT_0611_merijndebakker -- put the decision info in the matrix
        probs = tree(i,5:4+c);
        treeInfo(end+1,:) = [i, 0, 0, tree(i,1),probs];
        treeInfo(end+1,:) = [i, 0, 0, tree(i,1),probs];
